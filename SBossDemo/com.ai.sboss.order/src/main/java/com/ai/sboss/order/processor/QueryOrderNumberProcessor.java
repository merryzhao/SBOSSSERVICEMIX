...
/**
 * 该processor用于构造 查询CRM7中订单列表数量 的参数信息，以便route的下一步能够正常调用CRM的订单列表查询接口
 * @author yinwenjie
 */
@Component("queryOrderNumberProcessor")
public class QueryOrderNumberProcessor implements IBasicInProcessor {
	/**
	 * 这个常量是crm request的固定参数servicecode=queryOrderList
	 */
	private static final String CRM_SERVICE_CODE = "queryOrderNumber"; 
	 ...
	 * 日志信息
	 */
	private static final Log LOGGER = LogFactory.getLog(QueryOrderNumberProcessor.class);
	...
	public void process(Exchange exchange) throws Exception {
		...
		//向CRM发送参数了
		String urlParams = "bridgeEndpoint=true&servicecode=" + QueryOrderNumberProcessor.CRM_SERVICE_CODE + "&WEB_HUB_PARAMS=" + properParams;
		QueryOrderNumberProcessor.LOGGER.info("向CRM发送的参数为：" + urlParams);
	...
	public JSONObject getInputParam(Exchange exchange) throws Exception {
		...
		QueryOrderNumberProcessor.LOGGER.info("jetty URL 传来的参数信息：" + ret);
		String customerId = ret.getString("userId");
		...
		crmParamsJson_heade.put("Content-Type", QueryOrderNumberProcessor.CRM_SERVICE_HEAD_CONTENT_TYPE);
