 ...
 * @author yinwenjie
 */
@Component("queryOrderDetailProcessor")
public class QueryOrderDetailProcessor implements IBasicInProcessor {
	/**
	 * 日志
	 */
	private static final Log LOGGER = LogFactory.getLog(QueryOrderDetailProcessor.class);
	
	private static final String CRM_QUERY = "servicecode=queryOrderDetail&WEB_HUB_PARAMS={\"data\":<DATA>,\"header\":{\"Content-Type\":\"application/json\"}}";
	...
	public JSONObject getInputParam(Exchange exchange) throws Exception {
		...
		QueryOrderDetailProcessor.LOGGER.info("===============准备开始调用CRM的queryOrderDetail接口===============");
		QueryOrderDetailProcessor.LOGGER.info("customerOrderId = " + customerOrderId);
