...
public class QueryOfferingInfoConvertor implements IBasicOutProcessor {
...
//		JSONObject service_price = new JSONObject();
		...
		Double priceFinal = Double.parseDouble(priceTemp);
		logger.info("pricefinal=====>" + priceFinal);
		service_info.put("service_price", priceFinal);
		String billing_mode = "";
		...
		service_info.put("price_unit", unit);
...
//			service_info.put("service_thumbnail_url", service_thumbnail_url);
			...
			logger.info("queryOfferingInfoConvertor.price======>" + price);
			...
			logger.info("Chinese character value is " + specCharObject.getString("charSpecName"));
			if (specCharId == 25043195L){
				service_info.put("price_unit", valueString);
			}else if (specCharId == 25043196L){
				service_info.put("billing_mode", valueString);
			}else{
