...
/**
 * 利用camel向外部提供的查询行业流程可选任务节点列表（任务节点将按照编号进行排序，默认的排序）.的接口
 * @author yinwenjie
 */
@Component("queryJointByTradeidProcessor")
public class QueryJointByTradeidProcessor implements IBasicInProcessor {

	/**
	 * 日志
	 */
	private static final Log LOGGER = LogFactory.getLog(QueryJointByTradeidProcessor.class);
	...
	public void process(Exchange exchange) throws Exception {
		...
		try {
			...
			scope = httpDateParams.has("scope")?httpDateParams.getString("scope"):null;
			LOGGER.info("========得到参数：" + httpDateParams.toString());
		} catch(BizException e) {
			QueryJointByTradeidProcessor.LOGGER.error(e.getMessage(), e);
		...
		//2、================
		if(jsonEntity == null) {
			jsonEntity = this.queryOps.queryJointByTradeid(tradeid, scope);
