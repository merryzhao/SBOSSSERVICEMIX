...
/**
 * 利用camel向外部提供的删除指定任务的接口<br>
 * 这个接口虽然提供给了客户端，但是建议客户端慎重调用
 * @author yinwenjie
 */
@Component("deleteJointItemProcessor")
public class DeleteJointItemProcessor implements IBasicInProcessor {
	/**
	 * 日志
	 */
	...
	public void process(Exchange exchange) throws Exception {
		...
		try {
			...
			jointid = httpDateParams.getString("jointid");
			LOGGER.info("========得到参数：" + httpDateParams.toString());
		} catch(BizException e) {
			DeleteJointItemProcessor.LOGGER.error(e.getMessage(), e);
		...
		if(jsonEntity == null) {
			jsonEntity = this.deleteOps.deleteJointItem(jointid);
