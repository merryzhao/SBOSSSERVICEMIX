...
public class ModifyContentTagsFakeProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		Long contentID = JSONObject.fromObject(params).getJSONObject("data").getLong("content_id");
		JSONArray newtags = JSONObject.fromObject(params).getJSONObject("data").getJSONArray("content_tags");
		...
		try {
			...
			if (filecontent != null) {
				...
				for (int j = 0; j < contentlist.size(); ++j) {
					...
					Long content_id = content.getLong("content_id");
					if (content_id.equals(contentID)) {
						contentlist.getJSONObject(j).put("content_tags", newtags.toString());
